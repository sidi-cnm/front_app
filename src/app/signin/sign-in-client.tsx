...
function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden>
      <path fill="#4285F4" d="M23.49 12.27c0-.85-.07-1.47-.22-2.11H12v3.83h6.53c-.13.95-.83 2.39-2.39 3.36l-.02.11 3.47 2.69.24.02c2.2-2.03 3.66-5.02 3.66-8.9Z"/>
      <path fill="#34A853" d="M12 24c3.31 0 6.09-1.09 8.12-2.97l-3.87-3c-1.04.72-2.46 1.22-4.25 1.22-3.25 0-6.01-2.18-7-5.14l-.12.01-3.78 2.92-.05.11C3.05 21.53 7.18 24 12 24Z"/>
      <path fill="#FBBC05" d="M5 13.11A7.98 7.98 0 0 1 4.56 11c0-.73.13-1.45.35-2.11l-.01-.14-3.83-2.97-.13.06A12.001 12.001 0 0 0 0 11c0 1.96.47 3.82 1.3 5.45L5 13.11Z"/>
      <path fill="#EA4335" d="M12 4.74c2.3 0 3.86.98 4.75 1.8l3.47-3.39C18.07 1.03 15.31 0 12 0 7.18 0 3.05 2.47 1.16 5.75l3.75 2.99C5.9 6.78 8.76 4.74 12 4.74Z"/>
    </svg>
  );
}
...
function TinySwitch({
  ...
  return (
    ...
    >
      <span className={`h-3.5 w-6 rounded-full transition-colors ${checked ? "bg-emerald-500" : "bg-gray-300"}`}>
        <span className={`ml-[2px] mt-[2px] block h-2.5 w-2.5 rounded-full bg-white transition-transform ${checked ? "translate-x-3" : ""}`}/>
      </span>
      {label}
    </button>
  );
        ...
        <div className="-mt-24 sm:-mt-28 lg:-mt-32 grid place-items-center relative z-10">
          <div className="w-[380px] rounded-[18px] bg-white p-6 shadow-[0_20px_50px_rgba(0,0,0,0.12)] ring-1 ring-black/5">
            ...
            <div className="mt-3 flex items-center justify-center gap-3">
              <SocialButton label="Sign in with Facebook" onClick={() => oauth("facebook")} disabled={loading}>
                <Facebook className="h-4 w-4" />
              </SocialButton>
              <SocialButton label="Sign in with Apple" onClick={() => oauth("apple")} disabled={loading}>
                <Apple className="h-4 w-4" />
              </SocialButton>
              <SocialButton label="Sign in with Google" onClick={() => oauth("google")} disabled={loading}>
                <GoogleMark />
              </SocialButton>
            </div>
