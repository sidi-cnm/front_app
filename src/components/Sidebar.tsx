...
const items = [
  ...
  { href: "/sign-in", label: "Sign Out", icon: LogOut },
];
        ...
        {items.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex items-center gap-3 rounded-xl px-3 py-2 text-sm ${active ? "bg-white shadow-card text-dark" : "text-gray-600 hover:bg-white/70"}`}
            >
              <Icon size={18} />
              {label}
            </Link>
